...
public abstract class StratumServer
{
    ...
    protected async Task OnRequestAsync(StratumConnection connection, JsonRpcRequest request, CancellationToken ct)
    {
        ...
        if(banManager != null && await banManager?.IsBanned(connection.RemoteEndpoint.Address) == true)
        {
    ...
    protected async Task OnConnectionError(StratumConnection connection, Exception ex)
    {
        ...
        switch(ex)
        {
            ...
            case JsonException jsonEx:
                ...
                if(clusterConfig.Banning?.BanOnJunkReceive.HasValue == false || clusterConfig.Banning?.BanOnJunkReceive == true)
                {
                    ...
                    await banManager?.Ban(connection.RemoteEndpoint.Address, BanReason.Junk, TimeSpan.FromMinutes(3));
                }
                break;
            ...
            case AuthenticationException authEx:
                ...
                if(clusterConfig.Banning?.BanOnJunkReceive.HasValue == false || clusterConfig.Banning?.BanOnJunkReceive == true)
                {
                    ...
                    await banManager?.Ban(connection.RemoteEndpoint.Address, BanReason.InvalidTLSHandshake, TimeSpan.FromMinutes(3));
                }
                break;
    ...
    private bool DisconnectIfBanned(Socket socket, IPEndPoint remoteEndpoint)
    {
        ...
        bool isBanned = Task<bool>.Run(async () =>
        {
            return await banManager.IsBanned(remoteEndpoint.Address);
