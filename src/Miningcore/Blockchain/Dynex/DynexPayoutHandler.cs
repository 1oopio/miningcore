    ...
    IPayoutHandler
{
    ...
    private async Task<bool> HandleTransferResponseAsync(RpcResponse<TransferResponse> response, params Balance[] balances)
    {
        ...
        {
            logger.Error(() => $"[{LogCategory}] Daemon command '{DynexWalletCommands.Transfer}' returned error: {response.Error.Message} code {response.Error.Code}");

            NotifyPayoutFailure(poolConfig.Id, balances, $"Daemon command '{DynexWalletCommands.Transfer}' returned error: {response.Error.Message} code {response.Error.Code}", null);
            return false;
        }
    }
    ...
    private async Task<bool> HandleTransferResponseAsync(RpcResponse<TransferSplitResponse> response, params Balance[] balances)
    {
        ...
        {
            logger.Error(() => $"[{LogCategory}] Daemon command '{DynexWalletCommands.TransferSplit}' returned error: {response.Error.Message} code {response.Error.Code}");

            NotifyPayoutFailure(poolConfig.Id, balances, $"Daemon command '{DynexWalletCommands.TransferSplit}' returned error: {response.Error.Message} code {response.Error.Code}", null);
            return false;
        }
    }
            ...
            if(!string.IsNullOrEmpty(info.NetType))
            {
                switch(info.NetType.ToLower())
                {
                    case "mainnet":
                        ...
                        break;
                    case "stagenet":
                        networkType = DynexNetworkType.Stage;
                        break;
                    case "testnet":
                ...
                networkType = info.IsTestnet ? DynexNetworkType.Test : DynexNetworkType.Main;
        }
    }
    ...
    private async Task<bool> PayoutBatch(Balance[] balances, CancellationToken ct)
    {
        ...
        // send command
        var transferResponse = await rpcClientWallet.ExecuteAsync<TransferResponse>(logger, DynexWalletCommands.Transfer, ct, request);
        ...
        if(transferResponse.Error?.Code == -4)
        {
            ...
            {
                logger.Error(() => $"[{LogCategory}] Daemon command '{DynexWalletCommands.Transfer}' returned error: {transferResponse.Error.Message} code {transferResponse.Error.Code}");
                logger.Info(() => $"[{LogCategory}] Retrying transfer using {DynexWalletCommands.TransferSplit}");

                var transferSplitResponse = await rpcClientWallet.ExecuteAsync<TransferSplitResponse>(logger, DynexWalletCommands.TransferSplit, ct, request);
    ...
    private async Task<bool> PayoutToPaymentId(Balance balance, CancellationToken ct)
    {
        ...
        // send command
        var result = await rpcClientWallet.ExecuteAsync<TransferResponse>(logger, DynexWalletCommands.Transfer, ct, request);
        ...
        if(walletSupportsTransferSplit)
        {
            ...
            if(result.Error?.Code == -4)
            {
                ...
                result = await rpcClientWallet.ExecuteAsync<TransferResponse>(logger, DynexWalletCommands.TransferSplit, ct, request);
            }
        }
    ...
    public async Task ConfigureAsync(ClusterConfig cc, PoolConfig pc, CancellationToken ct)
    {
        ...
        var response = await rpcClientWallet.ExecuteAsync<TransferResponse>(logger, DynexWalletCommands.TransferSplit, ct);
        walletSupportsTransferSplit = response.Error.Code != DynexConstants.MoneroRpcMethodNotFound;
    }
    ...
    public async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        balances = balances
            .Where(x =>
            {
                ...
                var addressPrefix = DynexBindings.DecodeAddress(address);
                var addressIntegratedPrefix = DynexBindings.DecodeIntegratedAddress(address);
        ...
        // save wallet
        await rpcClientWallet.ExecuteAsync<JToken>(logger, DynexWalletCommands.Store, ct);
    }
