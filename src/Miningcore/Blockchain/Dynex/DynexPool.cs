...
using Miningcore.Blockchain.Dynex.StratumResponses;
...
public class DynexPool : PoolBase
{
    public DynexPool(IComponentContext ctx,
    ...
    private DynexJobManager manager;
    ...
    private async Task OnLoginAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var loginRequest = request.ParamsAs<DynexLoginRequest>();
        ...
        if(index != -1)
        {
            ...
            // validate
            if(!string.IsNullOrEmpty(paymentId) && paymentId.Length != DynexConstants.PaymentIdHexLength)
        ...
        if(context.IsAuthorized)
        {
            ...
            // respond
            var loginResponse = new DynexLoginResponse
            {
    ...
    private async Task OnGetJobAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var getJobRequest = request.ParamsAs<DynexGetJobRequest>();
    ...
    private DynexJobParams CreateWorkerJob(StratumConnection connection)
    {
        var context = connection.ContextAs<DynexWorkerContext>();
        var job = new DynexWorkerJob(NextJobId(), context.Difficulty);
    ...
    private async Task OnSubmitAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // check request
            var submitRequest = request.ParamsAs<DynexSubmitShareRequest>();
            ...
            await connection.RespondAsync(new DynexResponseBase(), request.Id);
    ...
    private async Task OnNewJobAsync()
    {
        ...
        await Guard(() => ForEachMinerAsync(async (connection, ct) =>
        {
            // send job
            var job = CreateWorkerJob(connection);
            await connection.NotifyAsync(DynexStratumMethods.JobNotify, job);
        }));
    }
    ...
    {
        manager = ctx.Resolve<DynexJobManager>();
        ...
        Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                case DynexStratumMethods.Login:
                    await OnLoginAsync(connection, tsRequest);
                    break;

                case DynexStratumMethods.GetJob:
