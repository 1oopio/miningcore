...
public class DynexJobManager : JobManagerBase<DynexJob>
{
    ...
    protected async Task<bool> UpdateJob(CancellationToken ct, string via = null, string json = null)
    {
        try
        {
            ...
            if(isNew)
            {
                ...
                // init job
                job = new DynexJob(blockTemplate, instanceId, NextJobId(), coin, poolConfig, clusterConfig, newHash, randomXRealm);
                currentJob = job;
    ...
    {
        var response = await rpc.ExecuteAsync<GetInfoResponse>(logger, DynexCommands.GetInfo, ct);
    ...
    private async Task UpdateNetworkStatsAsync(CancellationToken ct)
    {
        try
        {
            var response = await rpc.ExecuteAsync(logger, DynexCommands.GetInfo, ct);
    ...
    protected override async Task<bool> AreDaemonsHealthyAsync(CancellationToken ct)
    {
        ...
        {
            // test wallet daemons
            var responses2 = await walletRpc.ExecuteAsync<object>(logger, DynexWalletCommands.GetAddress, ct);
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        ...
        // coin config
        var coin = poolConfig.Template.As<DynexCoinTemplate>();
        var infoResponse = await rpc.ExecuteAsync(logger, DynexCommands.GetInfo, ct);
        ...
        if(clusterConfig.PaymentProcessing?.Enabled == true && poolConfig.PaymentProcessing?.Enabled == true)
        {
            var addressResponse = await walletRpc.ExecuteAsync<GetAddressResponse>(logger, DynexWalletCommands.GetAddress, ct);
        ...
        // address validation
        poolAddressBase58Prefix = DynexBindings.DecodeAddress(poolConfig.Address);
