...
public class DynexJob
{
    public DynexJob(GetBlockTemplateResponse blockTemplate, byte[] instanceId, string jobId,
        DynexCoinTemplate coin, PoolConfig poolConfig, ClusterConfig clusterConfig, string prevHash, string randomXRealm)
    {
    ...
    private string EncodeBlob(uint workerExtraNonce)
    {
        ...
        return DynexBindings.ConvertBlob(blob, blobTemplate.Length).ToHexString();
    }
    ...
    private void ComputeBlockHash(ReadOnlySpan<byte> blobConverted, Span<byte> result)
    {
        ...
        DynexBindings.CryptonightHashFast(block, result);
    }
    ...
    public (Share Share, string BlobHex) ProcessShare(string nonce, uint workerExtraNonce, string workerHash, StratumConnection worker)
    {
        ...
        // convert
        var blobConverted = DynexBindings.ConvertBlob(blob, blobTemplate.Length);
