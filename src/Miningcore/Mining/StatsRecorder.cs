...
public class StatsRecorder : BackgroundService
{
    ...
    private async Task UpdatePoolHashratesAsync(string poolId, DateTime now, CancellationToken ct)
    {
    ...
    private async Task UpdateAsync(CancellationToken ct)
    {
        ...
        do
        {
            try
            {
                var now = clock.Now;
                await Task.WhenAll(
                    pools.Select(pool => UpdatePoolHashratesAsync(pool.Key, now, ct)).ToArray()
                );
            }
