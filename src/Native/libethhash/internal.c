...
uint64_t ethash_get_datasize(uint64_t const block_number, uint64_t epoch_length)
{
	assert(block_number / epoch_length < 2048);
	return dag_sizes[block_number / epoch_length];
}

uint64_t ethash_get_cachesize(uint64_t const block_number, uint64_t epoch_length)
{
	assert(block_number / epoch_length < 2048);
...
ethash_h256_t ethash_get_seedhash(uint64_t block_number, uint64_t epoch_length)
{
	ethash_h256_t ret;
...
ethash_light_t ethash_light_new(uint64_t block_number, uint64_t epoch_length)
{
	ethash_h256_t seedhash = ethash_get_seedhash(block_number, epoch_length);
	ethash_light_t ret;
	ret = ethash_light_new_internal(ethash_get_cachesize(block_number, epoch_length), &seedhash);
...
ethash_full_t ethash_full_new(ethash_light_t light, ethash_callback_t callback, uint64_t epoch_length)
{
	char strbuf[256];
